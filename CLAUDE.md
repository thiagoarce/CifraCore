...
Fase 1 (fundação) concluída — T1–T7 feitos (T8/T9 são follow-ups anotados no tasks.md da 001). Fase 2 (importação) **concluída** — T1–T5 feitos. Fase 3 (catálogo e UI) **concluída** — T1–T4 feitos (design system, AppShell/Dashboard, tela da música, edição — ver `specs/003-catalogo-ui/tasks.md` para os desvios de arquitetura registrados, incluindo dois bugs reais de race condition encontrados e corrigidos em `invalidate()`+`goto()` no SvelteKit). Skill `ui-ux-pro-max` instalada em `.claude/skills/` e usada pra fundamentar as decisões de design da fase 3. Próximo passo: escolher a próxima feature (`specs/004-inteligencia-musical` é a sequência natural do roadmap — acordes, transposição, unroll) e ler seu spec/plan/tasks antes de codar.
