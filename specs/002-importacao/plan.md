...
- **`batch-import.js`:** Node puro na raiz, usa `@supabase/supabase-js` com `SUPABASE_SERVICE_KEY` (env) e a RPC `import_song` (mesma transação da T2/T4 — o `service_role` bypassa RLS, mas a lógica de gravação não é reimplementada). Formato do `repertorio.txt`: uma entrada por linha, `Título; Artista; URL-ou-ARQUIVO.txt`. Linhas iniciadas com `#` são comentário. URLs passam pela Edge Function; caminhos locais passam pelo parser. Grava direto (sem HITL — uso consciente de bootstrap, documentado no help do script).
  - **Invocação real:** `node --experimental-strip-types batch-import.js <repertorio.txt> --band <band_id>` (ou `npm run batch-import -- ...`) — desvio deliberado do `node batch-import.js repertorio.txt` citado literalmente no spec R5. Entradas locais precisam do **mesmo** `parseChordSheet` do app; como Node não executa `.ts` nativamente, o script usa a flag nativa `--experimental-strip-types` (estável desde Node 22.6) para importar `src/lib/utils/chordSheetParser.ts` diretamente, em vez de manter uma terceira cópia da lógica de parsing (já são duas — frontend e `_shared/` da Edge Function — e divergirem é exatamente o tipo de bug já encontrado uma vez nesta feature). O `import type` de `$lib/types/ast` dentro desse arquivo é apagado na transpilação e nunca precisa ser resolvido em runtime, então o alias `$lib` não é um problema.
