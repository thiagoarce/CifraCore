...
- **Dados por URL (`songs/[id]`, `bands/[id]`) via load function com o cliente Supabase de `locals`/`parent()`** — RLS garante o escopo, a UI não filtra segurança, só exibe. **Desvio para o dashboard:** a lista de músicas do dashboard NÃO pode vir de um load — a "banda ativa" (`$currentBand`) só existe no `localStorage` do navegador, o servidor não tem como saber qual banda mostrar na primeira resposta (não há cookie/query param codificando isso). Por isso o dashboard busca as músicas **client-side**, num `$effect` que reage a `$currentBand` (mesmo padrão já usado no dedupe por URL da 002).
- **Todo fluxo que muda estado e navega em seguida vira form action, nunca `invalidate()`+`goto()` manual no client** — ver bug abaixo. Isso já vale para: login, cadastro, logout, criar banda, sair da banda. Fluxos que mudam estado mas **não navegam** (convidar membro, importar música) continuam client-side normalmente — só a combinação "muda estado + navega" tem a corrida.
